...
if 'VCAP_SERVICES' in os.environ:
    ...
    # Go through all of the keys in the source Redis instance and copy them to
    # the destination Redis instance.
    
    # Using the SCAN method here as opposed to KEYS (*) due to this being run in
    # a production system - Python Redis provides an iterable version to keep
    # track of the cursor for you.
    ...
    for keys in r_source.scan_iter():
        for key in keys:
            try:
                ...
                if data:
                    # Retrieve the TTL (expiration) of the key so we maintain
                    # that information - returned in seconds:
                    # https://redis.io/commands/ttl
                    ttl = r_source.ttl(key)

                    # TTL can be -2 in the case of a key not existing, or -1 for
                    # no expiration; in either case, set it to 0, which will
                    # mean no expiration when the key is restored in the new
                    # instance.
                    if ttl < 0:
                        ttl = 0

                    # RESTORE the serialized key with its current TTL, which is
                    # specified in milliseconds: https://redis.io/commands/restore
                    r_destination.restore(key, ttl * 1000, data)

                    logging.info('Successfully migrated key {key}!'.format(key=key))
            except:
                logging.error('Could not migrate key {key}!'.format(key=key))

    # Now monitor the source instance for new writes and copy them over to the
    # destination instance while this app runs.

    # TODO:  Finish this and watch for SET or SETEX commands to mirror them on
    ...
    logging.error('Unable to retrieve Redis connection information; please check that the services are bound.')
